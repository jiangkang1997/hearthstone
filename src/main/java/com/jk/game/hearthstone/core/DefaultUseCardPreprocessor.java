...
public class DefaultUseCardPreprocessor implements UseCardPreprocessor {
    ...
    public void processBeforePlay(Desktop desktop, Card card, Organism target) throws IllegalOperationException {
        ...
        //目标合法性
        targetLegalityCheck(desktop,card, target);
        //随从数量合法性
        minionNumLegalityCheck(desktop,card, target);
    }

    private void powerLegalityCheck(Desktop desktop, Card card, Organism target) throws IllegalOperationException{
        int cost = card.getCost();
        Player player = card.getPlayerType() == PlayerType.PLAYER_TYPE_MAIN ? desktop.getMainPlayer() : desktop.getSecondPlayer();
    ...
    private void targetLegalityCheck(Desktop desktop, Card card, Organism target) throws IllegalOperationException{
        ...
        //todo: 英雄技能和指向性法术无法对魔免随从使用
    }

    private void minionNumLegalityCheck(Desktop desktop, Card card, Organism target) throws IllegalOperationException{
        List<Minion> minions = card.getPlayerType() == PlayerType.PLAYER_TYPE_MAIN ? desktop.getMainMinions() : desktop.getSecondMinions();
